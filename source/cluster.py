...
class Cluster(object) :
    ...
    def centroid(self) :
        ...
        labels = []
        n = len(self.points)
        d = len(self.points[0].attrs)
        for p in self.points :
            labels.append(p.label)

        common_label = stats.mode(labels)[0]

        sum_array = np.zeros(d)

        for point in self.points:
            sum_array = np.add(sum_array, point.attrs)
        for i in range(d):
            sum_array[i] = sum_array[i]/n

        centroid = Point("centroid", common_label, sum_array)
    ...
    def medoid(self) :
        ...
        # part 4a: implement
        n = len(self.points)
        point_dists = [0 for i in range(n)] # init dist array
        for i in range(n):
            curr_point = self.points[i]
            for j in range(n):
                point_dists[i] += curr_point.distance(self.points[j]) # compute distances
        medoid = self.points[np.argmin(point_dists)]
        return medoid
        ### ========== TODO : END ========== ###
